...
test('云暴山碎保留官方锤架式身份、单一规范敌人和武形秘仪中文协议', () => {
  ...
  assert.deepEqual(card.sections.enemy, ['- 土星 Tethys 刺杀 · Sargas Ruk 将军（11.06%）'])
...
test('刺杀 Boss 掉落协议跨至少三个其他 Mod 保留星球、节点、Boss 和概率语义', () => {
  assert.match(enemyLine('Vulcan Blitz', 'Kela De Thaym'), /赛德娜 Merrow 刺杀 · Kela De Thaym（11\.11%）/)
  assert.match(enemyLine('Intensify', 'Alad V'), /木星 Themisto 刺杀 · Alad V（2\.01%）/)
  assert.match(enemyLine('Pressure Point', 'Ambulas'), /冥王星 Hades 刺杀 · Ambulas（60\.704%）/)
})
...
test('全部已录入刺杀 Boss Mod 掉落都保留完整上下文并按敌人去重', () => {
  ...
  for (const entry of affected) {
    ...
    for (const method of methods) {
      ...
      assert.match(matches[0], /（(?:必定获得，)?\d+(?:\.\d+)?%）$/)
      checked += 1
    }
  }
