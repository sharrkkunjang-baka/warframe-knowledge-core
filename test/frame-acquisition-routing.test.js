...
test('Nokko 通过赏金、NPC、地点、任务和铁离石变量渲染', () => {
  ...
  assert.match(result.description, /整套蓝图：在气密舱的夜帽处兑换，需要720个铁离石/)
...
test('require 区分声望与货币并通过 NPC 地点变量渲染', () => {
  ...
  assert.match(createKnowledgeCore().getAcquisition('Dante').description, /资源数量加成：承载体毛细血管不受影响[\s\S]*兑换成本固定为540个承载体毛细血管，不会因加成改变[\s\S]*整套蓝图：在解剖圣所的洛德处兑换，需要540个承载体毛细血管$/)
  ...
  assert.match(createKnowledgeCore().getAcquisition('Nokko').description, /资源数量加成：[\s\S]*兑换成本固定为720个铁离石，不会因加成改变[\s\S]*整套蓝图：在气密舱的夜帽处兑换，需要720个铁离石$/)
  ...
  assert.match(baruukResult.description, /^福尔图娜的Little Duck：总图需要 2级声望，部件蓝图需要 3级声望兑换/)
  ...
  assert.match(hildrynResult.description, /在福尔图娜找Little Duck 2级（Agent）声望兑换/)
