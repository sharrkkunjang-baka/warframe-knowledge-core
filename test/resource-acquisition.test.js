...
test('奥布山谷全量已发布资源按族结构化且仅保留两项证据不足待审', () => {
  ...
  for (const canonical of ['Gyromag Systems', 'Atmo Systems', 'Repeller Systems']) {
    const methods = core.getResourceAcquisition(canonical).structuredMethods
    const heists = methods.filter(method => method.type === 'heist-reward')
    assert.equal(heists.length, 2, canonical)
    assert.equal(new Set(heists.map(method => JSON.stringify([method.locationId, method.rotation, method.chance]))).size, heists.length, canonical)
...
test('钢铁精华与堕落全息密钥使用结构化审核来源', () => {
  ...
  const fragmented = steel.structuredMethods.find(method => method.sourceEntityId === 'enemy.the-fragmented-one')
  assert.equal(fragmented.sourceDisplayName, '接肢至尊')
  assert.deepEqual(fragmented.requirements, { type: 'mode', modeId: 'steel-path' })
  assert.deepEqual(fragmented.requirementLines, ['仅钢铁之路模式掉落'])
  const steelCard = core.getAcquisitionCard('钢铁精华')
  assert.deepEqual(steelCard.sectionNotes.enemy, ['仅钢铁之路模式掉落'])
  assert.equal(JSON.stringify(steelCard).split('仅钢铁之路模式掉落').length - 1, 1)
  assert.doesNotMatch(JSON.stringify(steelCard), /碎片一号|The Fragmented One|rawTable|sourceCanonical/)
  assert.ok(steelCard.sections.other.some(line => line.includes('每日钢铁之路侵袭')))
  assert.ok(steelCard.sections.other.every(line => !line.includes('仅钢铁之路模式掉落')))
...
test('所有已发布材料都有可生图方法且用户视图不泄漏上游字段', () => {
  const core = coreModule.createKnowledgeCore({ approvedOnly: false })
  const forbidden = /rawRows?|rawTable|dropTable|sourceCanonical|sourceEntityId|locationId|missionTypeId|reviewStatus|provenance|probabilityArray|DTO|formatter|DT_[A-Z_]+/i
  const failures = []
  for (const resource of core.listResources().filter(item => item.reviewStatus === 'approved')) {
    const result = core.getResourceAcquisition(resource.canonical)
    const card = core.getAcquisitionCard(resource.canonical)
    if (!result?.structuredMethods.length) failures.push(`${resource.canonical}: missing structuredMethods`)
    if (!card || !Object.values(card.sections || {}).flat().length) failures.push(`${resource.canonical}: text-only`)
    const visible = JSON.stringify({ text: result?.text, sections: card?.sections })
    if (forbidden.test(visible) || visible.split(/\\n/).some(line => /^\s*\|.*\|\s*$/.test(line))) failures.push(`${resource.canonical}: raw-upstream`)
  }
  assert.deepEqual(failures, [])
})

test('敌人来源质量门拒绝碎片、任务对象与未注册稳定实体', () => {
  const { inspectMethod } = require('../scripts/audit-acquisition-quality')
  const { loadEntityRegistries } = require('../src/entities')
  const registries = loadEntityRegistries()
  const base = { type: 'enemy-drop', requirements: { type: 'none' }, requirementLines: [], reviewStatus: 'approved' }
  assert.ok(inspectMethod({ ...base, sourceDisplayName: '碎片一号' }, registries).includes('suspicious-enemy-source'))
  assert.ok(inspectMethod({ ...base, sourceDisplayName: '任务对象 01' }, registries).includes('suspicious-enemy-source'))
  assert.ok(inspectMethod({ ...base, sourceEntityId: 'enemy.not-registered' }, registries).includes('enemy-source-not-in-registry'))
  assert.deepEqual(inspectMethod({ ...base, sourceEntityId: 'enemy.the-fragmented-one' }, registries), [])
})

test('全量获取质量门不允许已发布材料泄漏或只能文字输出', () => {
  ...
  assert.deepEqual(report.publishedIssues.filter(issue => issue.category === 'resource'), [])
