...
test('统一获取卡片按来源分区并使用审核变种家族', () => {
  ...
  assert.deepEqual(growingPower.sections.enemy, ['- 无赖魅影（概率获得，6.06%）']);
  ...
  assert.deepEqual(flameGland.sections.enemy, ['- 炉渣翻打鬣狗（概率获得，0.06%）', '- 回旋鬣狗（概率获得，0.06%）', '- 冰沼鬣狗（概率获得，0.06%）', '- 奥布山谷航天站内的敌人（概率获得，0.047%）', '- 烈背鬣狗（概率获得，0.06%）']);
...
test('官方 Mod 快照为全部当前上游记录给出可用或排除状态', () => {
  ...
  const upstreamUniqueNames = new Set(new (require('warframe-items'))({ category: ['Mods'], i18n: ['zh'] }).map(mod => mod.uniqueName));
  const catalogRecords = [...core.officialCatalog.mods, ...core.officialCatalog.excludedMods];
  const catalogUniqueNames = new Set(catalogRecords.map(mod => mod.uniqueName));
  assert.equal(catalogRecords.length, catalogUniqueNames.size);
  assert.ok([...upstreamUniqueNames].every(uniqueName => catalogUniqueNames.has(uniqueName)));
  assert.ok(catalogUniqueNames.size >= upstreamUniqueNames.size);
...
test('current Wiki 已发布专家 Mod 必须进入生产目录和共享解析器', () => {
  for (const query of ['制衡prime', '制衡 Prime', 'Primed Counterbalance']) {
    assert.equal(core.resolveName(query)?.canonical, 'Primed Counterbalance');
    const mod = core.getOfficialMod(query);
    assert.equal(mod.uniqueName, '/Lotus/Upgrades/Mods/Shotgun/Expert/WeaponRecoilReductionModExpert');
    assert.equal(mod.status, 'complete');
    assert.equal(core.getOfficialModAvailability(query).status, 'available');
  }
  const entry = core.getAcquisition('制衡 Prime').entry;
  assert.equal(entry.reviewStatus, 'approved');
  assert.equal(entry.modAcquisition.generated.wiki.methods[0].sourceEntityId, 'npc.baro-ki-teer');
  assert.deepEqual(entry.modAcquisition.generated.wiki.methods[0].currency, [
    { currencyId: 'currency.credits', amount: 220000 },
    { currencyId: 'currency.orokin-ducats', amount: 300 }
  ]);
});
