...
test('统一获取卡片按来源分区并使用审核变种家族', () => {
  ...
  assert.deepEqual(growingPower.sections.enemy, ['- 无赖魅影（6.06%）']);
  ...
  assert.deepEqual(flameGland.sections.enemy, ['- 炉渣翻打鬣狗（0.06%）', '- 回旋鬣狗（0.06%）', '- 冰沼鬣狗（0.06%）', '- 奥布山谷航天站内的敌人（0.047%）', '- 烈背鬣狗（0.06%）']);
