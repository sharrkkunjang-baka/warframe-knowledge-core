...
test('全量货币加成同步保留 unknown 而不把缺证据武断写成无效', () => {
  ...
  const holokey = core.currencies.get('currency.corrupted-holokey')
  assert.equal(holokey.boosterEffects.resourceAmount, 'unaffected')
  assert.equal(holokey.boosterEffects.resourceDropChance, 'unaffected')
  assert.ok(holokey.boosterEffects.evidence.length >= 2)
})
