...
test('HTML table 保留 th/td、rowspan 与 colspan 语义', () => { const tables = extractHtmlTables(page.html); assert.equal(tables.length, 1); assert.equal(tables[0].rows[0].cells[0].tag, 'th'); assert.deepEqual(tableMatrix(tables[0]), [['Item Source','Reward','Reward'],['Item Source','Chance','Expected'],['Thrax Centurion','0.33%','299 Kills']]) })
