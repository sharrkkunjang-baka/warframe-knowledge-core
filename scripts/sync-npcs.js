...
function loadOfficialLanguages() {
  return {
    chinese: JSON.parse(fs.readFileSync(path.join(LANGUAGE_CACHE, 'languages.zh.json'), 'utf8')),
    english: JSON.parse(fs.readFileSync(path.join(LANGUAGE_CACHE, 'languages.en.json'), 'utf8'))
  }
}
function buildExactOfficialIndex(english, chinese) {
  const candidates = new Map()
  const nonEntityPrefixes = ['/Lotus/Language/Menu/', '/Lotus/Language/1999Passwords/']
  for (const [languageKey, canonical] of Object.entries(english)) {
    if (nonEntityPrefixes.some(prefix => languageKey.startsWith(prefix))) continue
    const displayName = String(chinese[languageKey] || '').trim()
    if (!displayName) continue
    const key = String(canonical || '').normalize('NFKC').trim().toLocaleLowerCase('en-US')
    if (!key) continue
    if (!candidates.has(key)) candidates.set(key, [])
    candidates.get(key).push({ languageKey, displayName })
  }
  const unique = new Map()
  for (const [key, matches] of candidates) {
    const names = [...new Set(matches.map(match => match.displayName))]
    if (names.length === 1) unique.set(key, matches.find(match => match.displayName === names[0]))
  }
  return unique
}
function localizedNpc(name, chinese, exactOfficial = new Map()) {
  const explicitKey = OFFICIAL_LANGUAGE_KEYS[name] || null
  const exact = exactOfficial.get(String(name).normalize('NFKC').trim().toLocaleLowerCase('en-US')) || null
  const languageKey = explicitKey || exact?.languageKey || null
  const displayName = explicitKey ? String(chinese[explicitKey] || '').trim() : AUDITED_ZH[name] || exact?.displayName || ''
  if (explicitKey && !displayName) throw new Error(`${name}: 官方语言键 ${explicitKey} 缺少简中值`)
...
function buildPlan(dbPath) {
  const resolved = resolveWikiDatabase(dbPath); const db = new ReadonlyWikiDatabase(resolved); const records = []; const seen = new Map(); const { chinese, english } = loadOfficialLanguages(); const exactOfficial = buildExactOfficialIndex(english, chinese)
  try {
    for (const [category, definition] of Object.entries(CATEGORY_PAGES)) for (const name of definition.names) {
      ...
      const npc = buildNpc(db, name, category, definition.locationId, chinese, exactOfficial); npc.categories = [category]; records.push(npc); seen.set(name, npc)
    }
    for (const name of [...ROOT_NPCS, ...listCharacterPages(resolved)]) if (!seen.has(name)) { const npc = buildNpc(db, name, null, null, chinese, exactOfficial); npc.categories = []; records.push(npc); seen.set(name, npc) }
  } finally { db.close() }
...
module.exports={CATEGORY_PAGES,ROOT_NPCS,LOCATION_OVERRIDES,AUDITED_ZH,OFFICIAL_LANGUAGE_KEYS,NPC_ROLES,buildExactOfficialIndex,localizedNpc,listCharacterPages,buildPlan,run}
